...
namespace ipcgull {
    ...
    // Per-thread slot holding the unique bus name of the D-Bus caller whose method
    // call is currently being dispatched on this thread. Set in gdbus_method_call
    // under server_lock and cleared via RAII when the dispatch scope exits, so it is
...
struct server::internal {
    ...
    static void gdbus_method_call(
        ...
        if (auto i = static_cast<std::weak_ptr<internal>*>(
            ...
            // sender is the caller's unique bus name, e.g. ":1.42". Thread it into
            // the per-thread current-caller slot so a method handler can read it via
            // ipcgull::current_caller() for the duration of this dispatch. The RAII
            // guard clears the slot on every exit path (including the throw path that
            // is turned into a D-Bus error below).
            ipcgull::g_current_caller = (sender ? sender : "");
            struct CallerGuard {
                ~CallerGuard() { ipcgull::g_current_caller.clear(); }
            } _caller_guard;
            // Debug: prove the caller identity is threaded through to the handler.
            g_debug("ipcgull: method call from %s", sender ? sender : "(null)");
